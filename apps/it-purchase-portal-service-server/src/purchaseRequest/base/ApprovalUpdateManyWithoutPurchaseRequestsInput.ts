/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApprovalWhereUniqueInput } from "../../approval/base/ApprovalWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ApprovalUpdateManyWithoutPurchaseRequestsInput {
  @Field(() => [ApprovalWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ApprovalWhereUniqueInput],
  })
  connect?: Array<ApprovalWhereUniqueInput>;

  @Field(() => [ApprovalWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ApprovalWhereUniqueInput],
  })
  disconnect?: Array<ApprovalWhereUniqueInput>;

  @Field(() => [ApprovalWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ApprovalWhereUniqueInput],
  })
  set?: Array<ApprovalWhereUniqueInput>;
}

export { ApprovalUpdateManyWithoutPurchaseRequestsInput as ApprovalUpdateManyWithoutPurchaseRequestsInput };
